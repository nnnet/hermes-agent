...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    description = (
        ...
        "LAST RESORT — only use when no native tool (or composition of native "
        "tools) can accomplish the task. Before calling this, scan your "
        "`tools` array for a dedicated tool that matches the action: "
        "mc_task_create, chief_spawn, kanban_create, mc_pipeline_run, "
        "mc_task_update, etc. all exist as first-class tool calls and MUST "
        "be preferred over wrapping a shell/SQL/HTTP call inside this "
        "sandbox. Native tools encode auth, schemas, dispatcher events, "
        "Aegis attestation, and audit trails — execute_code bypasses all "
        "of that.\n\n"
        "Legitimate use cases: 3+ generic tool calls (terminal/read_file/etc) "
        "with processing logic between them, filtering/reducing large tool "
        "outputs before they enter your context, conditional branching, "
        "loops (fetch N pages, process N files, retry on failure).\n\n"
        "Do NOT use for: anything a single native tool can do (creating a "
        "task, spawning a chief, updating MC status, listing pipelines, "
        "reading a file, sending a message). Use normal tool calls when: "
        "a native tool fits, single generic tool call with no processing, "
