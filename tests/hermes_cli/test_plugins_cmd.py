...
# ── _plugin_exists: bundled plugins with dir vs manifest-name mismatch ────


class TestPluginExistsBundledManifestName:
    """Bundled plugins whose folder name (``aegis_attestation``) differs from
    their canonical manifest ``name:`` (``aegis-attestation``) must still be
    discoverable by manifest name — otherwise the dashboard disable endpoint
    returns 400 ``Plugin 'X' is not installed or bundled.``
    """

    def _make_bundled(self, root: Path, dir_name: str, manifest_name: str) -> Path:
        plugins_dir = root / "plugins"
        plugins_dir.mkdir(parents=True, exist_ok=True)
        p = plugins_dir / dir_name
        p.mkdir()
    ...
    def test_dir_name_matches(self, tmp_path, monkeypatch):
        """Direct directory-name lookup still works (regression)."""
        from hermes_cli import plugins_cmd

        bundled = self._make_bundled(tmp_path, "my_plugin", "my-plugin")
        monkeypatch.setattr(
            "hermes_cli.plugins.get_bundled_plugins_dir", lambda: bundled
        )
        monkeypatch.setattr(plugins_cmd, "_plugins_dir", lambda: tmp_path / "user")
        assert plugins_cmd._plugin_exists("my_plugin") is True

    def test_manifest_name_matches_when_dir_differs(self, tmp_path, monkeypatch):
        """Reproduces the dashboard bug — disable sends ``aegis-attestation``
        (manifest name) but the directory is ``aegis_attestation``."""
        from hermes_cli import plugins_cmd

        bundled = self._make_bundled(
            tmp_path, "aegis_attestation", "aegis-attestation"
        )
        monkeypatch.setattr(
            "hermes_cli.plugins.get_bundled_plugins_dir", lambda: bundled
        )
        monkeypatch.setattr(plugins_cmd, "_plugins_dir", lambda: tmp_path / "user")
        assert plugins_cmd._plugin_exists("aegis-attestation") is True

    def test_unknown_name_returns_false(self, tmp_path, monkeypatch):
        """Bogus names still return False — no false positives."""
        from hermes_cli import plugins_cmd
        ...
        monkeypatch.setattr(plugins_cmd, "_plugins_dir", lambda: tmp_path / "user")
        assert plugins_cmd._plugin_exists("does-not-exist") is False
